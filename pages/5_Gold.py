...
st.set_page_config(page_title="Gold - Finance Track", page_icon="🥇", layout="wide", initial_sidebar_state="expanded")
inject_css(); T = get_theme(); data = get_data(); render_sidebar("Gold")
person = st.session_state.get("sb_person","Family")
df = build_investment_df(data["inv"], data["prices"], person)
sub = df[df["Asset_Class"]=="Gold"].copy() if not df.empty else __import__("pandas").DataFrame()
...
if not sub.empty and "Person" in sub.columns:
    ...
    sel_p = st.selectbox("View by Person", plist, key="Gold_pf")
    if sel_p != "Family": sub = sub[sub["Person"]==sel_p]

if sub.empty: info_box("No Gold investments found."); st.stop()
